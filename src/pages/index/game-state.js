...
addIpcMessageHandler("handleChanges", async (/**@type {[number,number,ArrayBuffer]}*/[width, height, changes]) => {
	...
	// YAMA BURADA BAŞLIYOR: Supabase'den eski pikselleri çekip tuvalin üstüne yazıyoruz
	try {
		const { data, error } = await supabase.from('pixels').select('*');
		if (data && data.length > 0) {
			console.log("Supabase'den " + data.length + " piksel yüklendi!");
			data.forEach(p => {
				const idx = p.x % WIDTH + (p.y % HEIGHT) * WIDTH;
				BOARD[idx] = parseInt(p.color);
				SOCKET_PIXELS[idx] = parseInt(p.color);
			});
...
export function sendServerMessage(name, args=undefined, event=undefined) {
	...
	// YAMA BURADA BAŞLIYOR: Eğer piksel koyuluyorsa, orijinaline ek olarak Supabase'e de kaydet
	if (name === "putPixel") {
		try {
			let pos, col;
			if (args.position !== undefined && args.colour !== undefined) {
			...
			if (pos !== undefined && col !== undefined) {
				...
				// Kendi ekranımızda anında göster ve Supabase'e kaydet!
				setPixelI(pos, col);
				supabase.from('pixels').upsert({ x: x, y: y, color: col.toString() }).then();
				
				// Cooldown süresini hemen bitir (0 bekleme için)
